...
class DirectGeocodingInteractor {
    ...
    let debouncer = Debouncer(debounceTime: 0.3) // 300 ms debounce

    func perform(for input: String, completion: @escaping (_ result: [SearchResultElement]?) -> Void) {
        // Debounce input by 300 ms, so that we're not refetching with every new character typed to avoid making too many unnecessary API calls
        debouncer.call {
